...
void CMenus::RenderSettingsTee7(CUIRect MainView)
{
	...
	CRenderTools::GetRenderTeeOffsetToRenderedTee(CAnimState::GetIdle(), &OwnSkinInfo, OffsetToMid);
	{
		...
		const vec2 TeesPosition = NormalSkinPreview.Center() + OffsetToMid;
		const vec2 DeltaPosition = Ui()->MousePos() - TeesPosition;
		...
		RenderTools()->RenderTee(CAnimState::GetIdle(), &OwnSkinInfo, TeeEmote, TeeDirection, TeesPosition);
