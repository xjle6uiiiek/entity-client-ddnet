...
void CMenus::RenderSettings(CUIRect MainView)
{
	...
	const char *apTabs[SETTINGS_LENGTH] = {
		...
		Localize("General"),
		Localize("Player"),
		(Client()->IsSixup() || g_Config.m_ClForceSevenSkin) ? "Tee 0.7" : Localize("Tee"),
		Localize("Appearance"),
		Localize("Controls"),
		Localize("Graphics"),
		Localize("Sound"),
	...
	else if(g_Config.m_UiSettingsPage == SETTINGS_TEE)
	{
		...
		if(Client()->IsSixup() || g_Config.m_ClForceSevenSkin)
...
void CMenus::RenderSettingsAppearance(CUIRect MainView)
{
	...
	else if(s_CurTab == APPEARANCE_TAB_CHAT)
	{
		...
		DoLine_ColorPicker(&s_FriendMessageColor, ColorPickerLineSize, ColorPickerLabelSize, ColorPickerLineSpacing, &RightView, Localize("Friend message"), &g_Config.m_ClFriendColor, ColorRGBA(1.0f, 0.137f, 0.137f), true, &g_Config.m_ClMessageFriend);
